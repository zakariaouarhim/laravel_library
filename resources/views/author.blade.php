...
@push('head')
    <x-seo.json-ld :schema="$schemas['person']" />
    <x-seo.json-ld :schema="$schemas['breadcrumbs']" />
    @if($schemas['itemList'])
        <x-seo.json-ld :schema="$schemas['itemList']" />
    @endif
@endpush
            ...
            <div class="hero-profile">
                <div class="hero-avatar">
                    @if($author->profile_image)
                        <img src="{{ asset('storage/' . $author->profile_image) }}" alt="{{ $author->name }}" width="120" height="120" loading="lazy" decoding="async">
                    @else
